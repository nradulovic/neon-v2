...
static struct my_queue_type NK_QUEUE_DEFINITE(uint8_t, QUEUE_SIZE) g_test_queue;

static void test_none_init(void)
{
    struct my_queue_type NK_QUEUE_DEFINITE(uint8_t, 16) my_queue;
    ...
    NK_QUEUE_DEFINITE_INIT(&my_queue);
}
...
static void test_empty_empty(void)
{
    ...
    NK_QUEUE_DEFINITE_EMPTY(&g_test_queue, &empty);
...
static void test_empty_size(void)
{
    ...
    NTESTSUITE_EXPECT_UINT(QUEUE_SIZE);
    NK_QUEUE_DEFINITE_SIZE(&g_test_queue, &size);
...
static void test_empty_is_full(void)
{
    ...
    NK_QUEUE_DEFINITE_IS_FULL(&g_test_queue, &is_full);
...
static void test_empty_is_empty(void)
{
    ...
    NTESTSUITE_EXPECT_BOOL(true);
    NK_QUEUE_DEFINITE_IS_EMPTY(&g_test_queue, &is_empty);
...
static void test_empty_fifo_head(void)
{
	...
	item = 234u;
    NK_QUEUE_DEFINITE_PUSH_FIFO(&g_test_queue, &item);
    NTESTSUITE_EXPECT_UINT(item);
    item = 0u;
    NK_QUEUE_DEFINITE_PEEK_HEAD(&g_test_queue, &item);
...
static void test_empty_fifo_tail(void)
{
    ...
    NTESTSUITE_EXPECT_UINT(item);
	item = 0u;
	NK_QUEUE_DEFINITE_PEEK_TAIL(&g_test_queue, &item);
	NTESTSUITE_ACTUAL_UINT(item);
}
...
static void test_empty_lifo_head(void)
{
    ...
    item = 234u;
    NK_QUEUE_DEFINITE_PUSH_LIFO(&g_test_queue, &item);
    NTESTSUITE_EXPECT_UINT(234u);
    item = 0u;
    NK_QUEUE_DEFINITE_PEEK_HEAD(&g_test_queue, &item);
    NTESTSUITE_ACTUAL_UINT(item);
}
...
{
    NK_QUEUE_DEFINITE_INIT(&g_test_queue);
}
